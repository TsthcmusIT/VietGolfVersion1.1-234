...
static const UIWindowLevel UIWindowLevelCFShareCircle = 1999.0f;  // Don't overlap system's alert.
...
-(id)init {
    self = [super init];
    if(self) {
        _sharers = @[[CFSharer facebook], [CFSharer twitter], [CFSharer mail]];
...
-(void)validateLayout {
    ...
    self.shareCircleContainerView.frame = CGRectMake(CGRectGetMidX(self.bounds) - CIRCLE_SIZE/2.0f, CGRectGetMidY(self.bounds) - CIRCLE_SIZE/2.0, CIRCLE_SIZE, CIRCLE_SIZE);
    self.shareCircleContainerView.layer.cornerRadius = CGRectGetWidth(self.shareCircleContainerView.frame) / 2.0f;
    ...
    if(self.isDragging) {
        ...
        self.introTextLayer.opacity = 0;
        self.touchLayer.opacity = 1.0f;
        ...
        self.introTextLayer.opacity = 1.0f;
        self.touchLayer.opacity = 0.2f;
        self.shareTitleLayer.string = @"";
...
-(void)setupSharers {
    ...
    for(int i = 0; i < self.sharers.count; i++) {
        ...
        float trig = section/([self numberOfVisibleSharers]/2.0f)*M_PI;
        float x = CIRCLE_SIZE/2.0f + cosf(trig)*PATH_SIZE/2.0f;
        float y = CIRCLE_SIZE/2.0f - sinf(trig)*PATH_SIZE/2.0f;
        sharerLayer.position = CGPointMake(x, y);
        sharerLayer.contents = (id)image.CGImage;
        sharerLayer.name = sharer._Name;
...
-(BOOL)circleEnclosesPoint:(CGPoint)point {
    if(pow(CIRCLE_SIZE/2.0f, 2) < (pow(point.x - CGRectGetMidX(self.shareCircleContainerView.bounds), 2.0f) + pow(point.y - CGRectGetMidY(self.shareCircleContainerView.bounds), 2.0f))) {
        return NO;
    }
    else {
        return YES;
...
#define GRAVITATIONAL_PULL 30.0f
...
-(CGPoint)touchLocationAtPoint:(CGPoint)point {
    ...
    // See if the new point is outside of the circle's radius.
    else if(pow(CIRCLE_SIZE/2.0f - TOUCH_SIZE/2.0f, 2) < (pow(point.x - CGRectGetMidX(self.shareCircleContainerView.bounds), 2.0f) + pow(point.y - CGRectGetMidY(self.shareCircleContainerView.bounds), 2.0f))) {
        ...
        point.x = CGRectGetMidX(self.shareCircleContainerView.bounds) - (CIRCLE_SIZE/2.0f - TOUCH_SIZE/2.0f) * cos(angle);
        point.y = CGRectGetMidY(self.shareCircleContainerView.bounds) + (CIRCLE_SIZE/2.0f - TOUCH_SIZE/2.0f) * sin(angle);
    }
